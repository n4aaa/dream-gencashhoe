    ...
    public BukkitMenuPaginated build(@NonNull HumanEntity humanEntity) {
        ...
        if (this.hoeItem.getItemStack().hasItemMeta()) {
            ...
            IntStream.range(0, lore.size()).forEach(i -> {
                ...
                bukkitMenuPaginated.addStorageItem(ItemBuilder.of(this.pluginConfig.iconLoreMenuTemplate.clone()).fixColors(new MapBuilder<String, Object>().put("line", !line.isEmpty() ? line : "&cPusta linia!").build()).toItemStack(), e -> {
                    if (e.getWhoClicked() instanceof Player) {
                        Player player = (Player) e.getWhoClicked();
